...
public class TCPChannelClient {
  ...
  private abstract class TCPSocket extends Thread {
    ...
    @Override
    public void run() {
      ALog.d(TAG, "Listening thread started...");

      // Receive connection to temporary variable first, so we don't block.
      Socket tempSocket = connect();
      BufferedReader in;

      ALog.d(TAG, "TCP connection established.");

      synchronized (rawSocketLock) {
        if (rawSocket != null) {
          ALog.e(TAG, "Socket already existed and will be replaced.");
        }
      ...
      ALog.v(TAG, "Execute onTCPConnected");
      executor.execute(new Runnable() {
        @Override
        public void run() {
          ALog.v(TAG, "Run onTCPConnected");
      ...
      while (true) {
        ...
        executor.execute(new Runnable() {
          @Override
          public void run() {
            ALog.v(TAG, "Receive: " + message);
            eventListener.onTCPMessage(message);
          }
        });
      }

      ALog.d(TAG, "Receiving thread exiting...");
  ...
  private class TCPSocketServer extends TCPSocket {
    ...
    public Socket connect() {
      ALog.d(TAG, "Listening on [" + address.getHostAddress() + "]:" + Integer.toString(port));
      ...
      synchronized (rawSocketLock) {
        if (serverSocket != null) {
          ALog.e(TAG, "Server rawSocket was already listening and new will be opened.");
        }
  ...
  private class TCPSocketClient extends TCPSocket {
    ...
    @Override
    public Socket connect() {
      ALog.d(TAG, "Connecting to [" + address.getHostAddress() + "]:" + Integer.toString(port));
