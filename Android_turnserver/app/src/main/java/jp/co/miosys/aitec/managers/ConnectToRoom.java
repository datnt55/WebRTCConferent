...
public class ConnectToRoom {
    ...
    public ConnectToRoom (Context context){
        mContext = context;
        initComponance();
    }


    public void initComponance() {
        // Get setting keys.
        ...
        keyprefRoom = mContext.getString(R.string.pref_room_key);
    ...
    private String sharedPrefGetString(
        ...
        if (useFromIntent) {
            String value = ((Activity)mContext).getIntent().getStringExtra(intentName);
            if (value != null) {
                return value;
            }
            return defaultValue;
    ...
    private boolean sharedPrefGetBoolean(
        ...
        if (useFromIntent) {
            return ((Activity)mContext).getIntent().getBooleanExtra(intentName, defaultValue);
        } else {
            String attributeName = mContext.getString(attributeId);
    ...
    private int sharedPrefGetInteger(
        ...
        if (useFromIntent) {
            return ((Activity)mContext).getIntent().getIntExtra(intentName, defaultValue);
        } else {
            String attributeName = mContext.getString(attributeId);
            ...
            try {
                ...
                ALog.e(TAG, "Wrong setting for: " + attributeName + ":" + value);
                return defaultValue;
            }
        }
    }
    ...
    public void connectToRoom(String roomId, boolean commandLineRun, boolean loopback,
        ...
        if (useValuesFromIntent) {
            videoWidth = ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_VIDEO_WIDTH, 0);
            videoHeight = ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_VIDEO_HEIGHT, 0);
        }
        if (videoWidth == 0 && videoHeight == 0) {
            ...
            if (dimensions.length == 2) {
                try {
                    ...
                    videoWidth = 0;
                    videoHeight = 0;
                    ALog.e(TAG, "Wrong video resolution setting: " + resolution);
                }
            }
        }
        ...
        int cameraFps = 0;
        if (useValuesFromIntent) {
            cameraFps = ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_VIDEO_FPS, 0);
        }
        if (cameraFps == 0) {
            ...
            if (fpsValues.length == 2) {
                try {
                    ...
                    cameraFps = 0;
                    ALog.e(TAG, "Wrong camera fps setting: " + fps);
                }
            }
        }
        ...
        if (useValuesFromIntent) {
            videoStartBitrate = ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_VIDEO_BITRATE, 0);
        }
        ...
        if (useValuesFromIntent) {
            audioStartBitrate = ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_AUDIO_BITRATE, 0);
        }
        ...
        ALog.d(TAG, "Connecting to room " + roomId + " at URL " + roomUrl);
        if (validateUrl(roomUrl)) {
            ...
            if (useValuesFromIntent) {
                if (((Activity)mContext).getIntent().hasExtra(CallActivity.EXTRA_VIDEO_FILE_AS_CAMERA)) {
                    String videoFileAsCamera =
                            ((Activity)mContext).getIntent().getStringExtra(CallActivity.EXTRA_VIDEO_FILE_AS_CAMERA);
                    intent.putExtra(CallActivity.EXTRA_VIDEO_FILE_AS_CAMERA, videoFileAsCamera);
                }

                if (((Activity)mContext).getIntent().hasExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE)) {
                    String saveRemoteVideoToFile =
                            ((Activity)mContext). getIntent().getStringExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE);
                    intent.putExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE, saveRemoteVideoToFile);
                }

                if (((Activity)mContext).getIntent().hasExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE_WIDTH)) {
                    int videoOutWidth =
                            ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE_WIDTH, 0);
                    intent.putExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE_WIDTH, videoOutWidth);
                }

                if (((Activity)mContext).getIntent().hasExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE_HEIGHT)) {
                    int videoOutHeight =
                            ((Activity)mContext).getIntent().getIntExtra(CallActivity.EXTRA_SAVE_REMOTE_VIDEO_TO_FILE_HEIGHT, 0);
