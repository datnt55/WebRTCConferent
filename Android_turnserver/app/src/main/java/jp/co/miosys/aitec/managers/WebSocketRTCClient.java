...
import jp.co.miosys.aitec.managers.RoomParametersFetcher.RoomParametersFetcherEvents;
...
public class WebSocketRTCClient implements AppRTCClient, WebSocketChannelClient.WebSocketChannelEvents {
    ...
    private void connectToRoomInternal() {
        ...
        ALog.d(TAG, "Connect to room: " + connectionUrl);
        roomState = ConnectionState.NEW;
        ...
        new RoomParametersFetcher(mContext, connectionUrl, null, callbacks).makeRequest();
    }

    // Disconnect from room and send bye messages - runs on a local looper thread.
    private void disconnectFromRoomInternal() {
        ALog.d(TAG, "Disconnect. Room state: " + roomState);
        if (roomState == ConnectionState.CONNECTED) {
            ALog.d(TAG, "Closing room.");
    ...
    private void signalingParametersReady(final SignalingParameters signalingParameters) {
        ALog.d(TAG, "Room connection completed.");
        ...
        if (!connectionParameters.loopback && !signalingParameters.initiator
            ...
            ALog.w(TAG, "No offer SDP in room response.");
        }
        initiator = signalingParameters.initiator;
        messageUrl = getMessageUrl(connectionParameters, signalingParameters);
        leaveUrl = getLeaveUrl(connectionParameters, signalingParameters);
        ALog.d(TAG, "Message URL: " + messageUrl);
        ALog.d(TAG, "Leave URL: " + leaveUrl);
    ...
    public void sendAnswerSdp(final SessionDescription sdp) {
        handler.post(new Runnable() {
            ...
            public void run() {
                if (connectionParameters.loopback) {
                    ALog.e(TAG, "Sending answer in loopback mode.");
                    return;
                }
                JSONObject json = new JSONObject();
                jsonPut(json, "sdp", sdp.description);
    ...
    public void onWebSocketMessage(final String msg) {
        if (wsClient.getState() != WebSocketChannelClient.WebSocketConnectionState.REGISTERED) {
            ALog.e(TAG, "Got WebSocket message in non registered state.");
            return;
        }
        try {
