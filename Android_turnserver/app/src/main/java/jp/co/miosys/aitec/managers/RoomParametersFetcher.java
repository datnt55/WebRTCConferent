...
import jp.co.miosys.aitec.utils.AsyncHttpURLConnection;
import jp.co.miosys.aitec.utils.AsyncHttpURLConnection.AsyncHttpEvents;
...
public class RoomParametersFetcher {
    ...
    public RoomParametersFetcher(Context context, String roomUrl, String roomMessage, final RoomParametersFetcherEvents events) {
        ...
        httpConnection =
                new AsyncHttpURLConnection("POST", roomUrl, roomMessage, new AsyncHttpEvents() {
                    ...
                    public void onHttpError(String errorMessage) {
                        ALog.e(TAG, "Room connection error: " + errorMessage);
    ...
    private void roomHttpResponseParse(String response) {
        ALog.d(TAG, "Room response: " + response);
        try {
            ...
            if (!initiator) {
                ...
                for (int i = 0; i < messages.length(); ++i) {
                    ...
                    ALog.d(TAG, "GAE->C #" + i + " : " + messageString);
                    if (messageType.equals("offer")) {
                        ...
                        ALog.e(TAG, "Unknown message: " + messageString);
                    }
                }
            }
            ALog.d(TAG, "RoomId: " + roomId + ". ClientId: " + clientId);
            ALog.d(TAG, "Initiator: " + initiator);
            ALog.d(TAG, "WSS url: " + wssUrl);
            ALog.d(TAG, "WSS POST url: " + wssPostUrl);

            LinkedList<PeerConnection.IceServer> iceServers =
                    iceServersFromPCConfigJSON(roomJson.getString("pc_config"));
            boolean isTurnPresent = false;
            for (PeerConnection.IceServer server : iceServers) {
                ALog.d(TAG, "IceServer: " + server);
            ...
            if (!isTurnPresent && !roomJson.optString("ice_server_url").isEmpty()) {
                ...
                for (PeerConnection.IceServer turnServer : turnServers) {
                    ALog.d(TAG, "TurnServer: " + turnServer);
...
//        ALog.d(TAG, "Request TURN from: " + url);
...
//        ALog.d(TAG, "TURN response: " + response);
////        JSONObject responseJSON = new JSONObject(response);
//
//        JSONObject responseJSON = new JSONObject(LoginActivity.test);
...
//            }
//        }
        turnServers.add(new PeerConnection.IceServer(Globals.TurnServerURI, Globals.TurnServerUser, Globals.TurnServerPass)); //"turn:157.7.209.73:1908", "vmio", "vm69vm69"));
        return turnServers;
    }
...
//        String testPcConfig = mContext.getResources().getString(R.string.test_ice_servers);
